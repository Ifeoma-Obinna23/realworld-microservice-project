...
pipeline {
    ...
    stages {
            ...
            steps {
                script {
                    withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'docker') {
                        sh "docker build -t ifeobi/frontendservice:latest ."
                    }
                }
            }
        }
        ...
        stage('Snyk SCA Test | Dependencies') {
            steps {
                sh "${SNYK_HOME}/snyk-linux test --docker ifeobi/frontendservice:latest || true" 
            }
        }
        ...
        stage('Push Microservice Docker Image') {
            steps {
                script {
                    withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'docker') {
                        sh "docker push ifeobi/frontendservice:latest "
                    }
                }
            }
        }
         ...
         stage('ZAP Dynamic Testing | DAST') {
             steps {
                 sshagent(['OWASP-Zap-Credential']) {
                     sh 'ssh -o StrictHostKeyChecking=no ubuntu@18.221.4.218 "docker run -t zaproxy/zap-weekly zap-baseline.py -t http://3.129.16.93:30000/" || true'
    ...
    always {
        ...
        slackSend channel: '#alert-ifeoma', //update and provide your channel name
